...
climate <- header_data %>%
  group_by(series) %>% 
  summarise(BIO1=mean(BIO1), BIO7=mean(BIO7), BIO12=mean(BIO12)) %>% 
  mutate(
    Temprt = as.vector(scale(BIO1, scale = TRUE)),
  #  BIO7_ = as.vector(scale(BIO7, scale = TRUE)),
...
write_csv(headers_PCA, "data/headers_PCA.csv")
